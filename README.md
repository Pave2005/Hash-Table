...
Профилирование программы будет производиться с помощью ```perf```. А именно ``perf stat`` для подсчета времени работы программы и ``perf record`` для выявления узких мест в программы, то есть функций, которые занимают наибольшую часть времени работы программы.

Для тестирования функции поиска запустим ее в цикле на 100'000'000 итераций.
...
Получено достаточно хорошее ускорение выполнения всей программы с помощью оптимизации хэш-функции.
...
Для оптимизации strcmp мы будем использовать функцию, написанную на ассемблере. Текст, который используется для поиска не имеет слов длины больше 16 символов, поэтому перепишем функцию сравнения для частного случая.
...
В данном случе функция вообще не  оптимизирует программу.

Скорее всего это происходит из-за того, что при инициализации хэш-таблицы на куче выделяется в два раза больше памяти.
...
С помощью интринсикс-функций мне удалось на 11% оптимизировать программу. При этом если посмотреть ассемблерный код программы то, он напоминает ассемблерную функцию, написанную мной.

### Результаты
| Версия программы | Ускорение от предыдущей версии | Ускорение от базовой |
|:----------------:|:------------------------------:|:---------:|
|CRC32 оптимизация |             1.16               | 1.16       |
|strcmp_asm |1.14|1.33|
|strcmp_simd| 1.13| 1.5|

## Вывод
Нам удалось ускорить функцию поиска в 1.5 раза, что является неплохим результатом. Возможно такой результат был получен, потому что я работал и тестировал программу на сервере. Используя такие инструменты, как perf и objdump, можно находить нужные объекты оптимизации и ускорять программу.
Главным при этом для оптимизации мне потребовалось переписать некоторые функции на ассемблере и воспользоваться simd-командами, что сделало код программы менее читаемым. В связи с этим, предлагается оценить результат оптимизации, рассчитывая его по следующей формуле:

result = (прирост производительности / количество строк платформозависимого кода) * 1000

В данной работе он составил result = (1.5 / 58) * 1000 = 26.
